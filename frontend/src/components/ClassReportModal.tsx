...
export default function ClassReportModal({ isOpen, onClose, user, classes, onSaved }: ClassReportModalProps) {
    const [edits, setEdits] = useState<Record<string, ClassMaster>>({});
    const [saving, setSaving] = useState(false);

    // Initialize edits when modal opens or classes change
    useEffect(() => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            // alert('クラス情報を更新しました！'); // Make less intrusive?
            onSaved();
            onClose();
        } catch (e) {
            console.error(e);
    ...
    return (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <div className="bg-white w-full max-w-2xl max-h-[90vh] rounded-2xl flex flex-col shadow-2xl">
                {/* Header */}
