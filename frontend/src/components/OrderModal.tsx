            ...
            <div className="bg-white w-full sm:max-w-lg h-[90vh] sm:h-auto sm:max-h-[90vh] rounded-t-2xl sm:rounded-2xl flex flex-col shadow-xl">
                ...
                <div className="p-4 border-t border-gray-100 bg-white sm:rounded-b-2xl">
                    <button
                        onClick={handleSave}
                        disabled={loading}
                        className="w-full bg-orange-500 text-white py-4 rounded-xl font-bold text-lg hover:bg-orange-600 flex items-center justify-center gap-2 shadow-lg ring-4 ring-orange-100 active:scale-95 transition-all"
                    >
                        {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <><Send className="w-5 h-5" /> 変更を申請する</>}
                    </button>
                </div>
            </div>
        </div>
